...
vec MoMa::thresh(vec feature, float high, float low, float min, float max)
{
    ...
    for (int t = 1; t < feature.n_rows; t++)
    {
        ...
        if (thresh(t - 1) == max)
        {
            ...
            if (feature(t) > high)
                thresh(t) = max;
            else if (feature(t) <= high && feature(t) > low)
                thresh(t) = max;
            else if (feature(t) <= low)
                thresh(t) = min;
        }
...
vec MoMa::peaks(vec feature, float width)
{
    ...
    uvec ur = find(dx > 0.0f);
    uvec uf = find(dx < 0.0f); // Rise and falls
    vec r;
    r.resize(ur.n_rows, 1);
    for (int k = 0; k < r.n_rows; k++)
        r(k) = ur(k);
    vec f;
    f.resize(uf.n_rows, 1);
    for (int k = 0; k < f.n_rows; k++)
        f(k) = uf(k);

    vec dr = r;
    dr.rows(1, r.n_rows - 1) = r.rows(1, r.n_rows - 1) - r.rows(0, r.n_rows - 2);
    ...
    vec df = f;
    df.rows(1, f.n_rows - 1) = f.rows(1, f.n_rows - 1) - f.rows(0, f.n_rows - 2);
...
arma::vec MoMa::nanmean(arma::mat v, int dim)
{
    ...
    if (dim == 0)
    {
        ...
        if (tmp.n_rows <= 0)
            return arma::datum::nan * ones(v.n_cols);
    ...
    else
    {
        ...
        if (tmp.n_cols <= 0)
            return arma::datum::nan * ones(v.n_rows);
