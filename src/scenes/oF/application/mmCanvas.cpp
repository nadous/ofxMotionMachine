...
Canvas::Canvas(SceneApp *app, string title, Position position, Position alignment, Canvas *relative, Canvas *parent, int group, bool minified) : ofxUISuperCanvas(title),
                                                                                                                                                 _app(app),
                                                                                                                                                 _relative(relative),
                                                                                                                                                 _parent(parent),
                                                                                                                                                 _group(group - 1),
...
Canvas::Canvas(SceneApp *app, std::string title, Canvas *parent, int group, bool minified) : ofxUISuperCanvas(title),
                                                                                             _app(app),
                                                                                             _relative(NULL),
                                                                                             _parent(parent),
                                                                                             _group(group - 1),
...
void Canvas::initCanvas()
{
        ...
        (relative == NULL && position == DEFAULT && _parent != NULL))
    {
        ...
        if (relativeY + relativeH + 10 + H < ofGetHeight() && _limit < 1)
        { // test window bottom limit

            position = BELOW;
            alignment = RIGHT;
        }
        else if (relativeX - 10 - W > 0 && _limit < 2)
        { // test window left limit
        ...
        else if (relativeY - 10 - H > 0 && _limit < 3)
        { // test window top limit
        ...
        else if (relativeX + relativeW + 10 + W < ofGetWidth() - 10 - mainCanvas[0]->getRect()->getWidth() - 10 && _limit < 4)
        { // test window right limit (taking into account the first Canvas canvas supposed to be on the top/right corner)
...
void Canvas::setPos(Position position, Position alignment, Canvas *relative)
{
    ...
    if (relative == NULL)
    { // Set positions relative to main OF Window
...
void Canvas::resetPositions()
{
    ...
    { //reopen canvas before re-initialization!
