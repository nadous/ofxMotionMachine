...
void ofPlot::updatePlot() {
  ...
  for (int f = 0; f < _figure.plot.size(); f++) {
    ...
    for (unsigned int n = 0; n < _figure.plot[f].data.nOfFrames(); n++) {
      ...
      _figure.plot[f].line.addVertex(x, y);  // Add vertex
    }
  }
}
...
void ofPlot::draw() {
  ...
  if (fabs(_figure.yBot - (float)(mFigureId + 1) * figWidth) > 1E-6)
    this->update();
  else if (fabs(curLowBoundTime - app->getLowBoundTime()) > 1E-6)
    this->update();
  else if (fabs(curHighBoundTime - app->getHighBoundTime()) > 1E-6)
    this->update();
  else if (curWidth != ofGetWidth())
    this->update();
