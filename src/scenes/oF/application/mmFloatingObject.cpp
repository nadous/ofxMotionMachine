...
*
* Created by Micka�l Tits on 08/03/2016
...
//--------------------------------------------------------------
FloatingObject::FloatingObject(SceneApp *app, std::string title, int x, int y, int width, int height, bool minified) : _app(app)
{
    saveIt = false;
    verbose = false;
    savedWidth = width;
...
FloatingObject::~FloatingObject()
{
    ...
    if (verbose)
        cout << "deleting object :" << title << endl;
    disableAllEvents();
    setFocus(false);
...
void FloatingObject::enableAppEvents()
{
    ...
    if (!appEnabled)
    {
        ...
        if (verbose)
            cout << "enabling _update of " << title << endl;
...
void FloatingObject::disableAppEvents()
{
    ...
    if (appEnabled)
    {
        ...
        if (verbose)
            cout << "disabling _update of " << title << endl;
...
void FloatingObject::_mouseMoved(ofMouseEventArgs &evt)
{
    ...
    if (isInsideClosingButton(evt.x, evt.y))
    { //Over Saving Button
    ...
    if (isInsideSavingButton(evt.x, evt.y))
    { //Over Saving Button
...
void FloatingObject::_mousePressed(ofMouseEventArgs &evt)
{
    ...
    //Focus on press
    if (isInsideObjectOrTitle(evt.x, evt.y))
    { //Inside overall object
...
void FloatingObject::_mouseReleased(ofMouseEventArgs &evt)
{
    ...
    if (!minified)
    {
        ...
        if ((ofGetElapsedTimef() - lastHitTime) < deltaTime) //double-clicked
        {
            ...
            mouseReleased(evt.x, evt.y, evt.button);
...
void FloatingObject::_draw(ofEventArgs &args)
{
    ...
    if (drawObject)
    {
        ...
        if (saveIt)
            ofSetColor(titleColor); //Draw without transparency if we are saving the object as an image
        ...
        // draw closing button
        if (!saveIt)
        { //don't draw the closing button if we are saving the object as an image

            ofSetColor(closingButtonColor, 200);
            if (closingButtonFocused)
                ofSetColor(closingButtonColor, 255);
            if (buttonSize > titleHeight)
                buttonSize = titleHeight;
        ...
        // draw saving button
        if (!saveIt)
        { //don't draw the saving button if we are saving the object as an image

            ofSetColor(savingButtonColor, 200);
            if (savingButtonFocused)
                ofSetColor(savingButtonColor, 255);
            if (buttonSize > titleHeight)
                buttonSize = titleHeight;
        ...
        if (!minified)
        {
            ...
            if (saveIt)
                ofSetColor(backGroundColor); //Draw without transparency if we are saving the object as an image
...
void FloatingObject::setFocus(bool val)
{
    ...
    if (_app->isExit)
        return; //Avoid bugs due to bad scenemode or canvas access

    if (val && hasFocus)
    { //Ensure the focused object is on front if it was not already (if another new object was opened for instance)
    ...
    if (val && isFocusFree)
    { //focus must be set, no other object already focused => we can set the focus
    ...
    if (!val && hasFocus)
    { //object is focused and must be unfocused
...
bool FloatingObject::isInsideObject(int mouseX, int mouseY)
{
    ...
    if (!isMinified())
        return ofRectangle(x, y, width, height).inside(mouseX, mouseY);
    else
        return false;
}
//--------------------------------------------------------------
bool FloatingObject::isInsideTitle(int mouseX, int mouseY)
{
    ...
    if (!isMinified())
        return ofRectangle(x, y - titleHeight, max(width, titleWidth), titleHeight).inside(mouseX, mouseY);
    else
        return ofRectangle(x, y - titleHeight, titleWidth, titleHeight).inside(mouseX, mouseY);
}
//--------------------------------------------------------------
bool FloatingObject::isInsideClosingButton(int mouseX, int mouseY)
{
    ...
    if (!isMinified())
        return ofRectangle(x + max(width, titleWidth) - 5 - buttonSize, y - (titleHeight + buttonSize) / 2, buttonSize, buttonSize).inside(mouseX, mouseY);
    else
        return ofRectangle(x + titleWidth - 5 - buttonSize, y - (titleHeight + buttonSize) / 2, buttonSize, buttonSize).inside(mouseX, mouseY);
}
//--------------------------------------------------------------
bool FloatingObject::isInsideSavingButton(int mouseX, int mouseY)
{
    ...
    if (!isMinified())
        return ofRectangle(x + max(width, titleWidth) - 5 - buttonSize - 5 - savingButtonWidth, y - (titleHeight + buttonSize) / 2, savingButtonWidth, buttonSize).inside(mouseX, mouseY);
    else
        return ofRectangle(x + titleWidth - 5 - buttonSize - 5 - savingButtonWidth, y - (titleHeight + buttonSize) / 2, savingButtonWidth, buttonSize).inside(mouseX, mouseY);
}
//--------------------------------------------------------------
bool FloatingObject::isOnCorner(int mouseX, int mouseY)
{

    if (!objectResizeEnabled)
        return false;
