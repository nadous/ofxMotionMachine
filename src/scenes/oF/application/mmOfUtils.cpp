...
const ofVec3f& getNodePosition(std::string nodename, MoMa::Track& track, int frame) {
...
const ofQuaternion& getNodeRotation(std::string nodename, MoMa::Track& track, int frame) {
  int nID = getNodeID(nodename, track);
  if (nID == -1)
    return ofQuaternion();
