...
*
* Created by Micka�l Tits on 05/03/2015
...
void C3dParser::load(string const &fileName, Track *track)
{
    ...
    if (EventIndicator == 12345)
    {
        ...
        if (Nevents > 0)
        {
            ...
            for (int i = 0; i < Nevents; i++)
            {
                fres = fread(Event[i].name, sizeof(char) * 4, 1, fid); // not sure if this works, never tried events (�a marche pas, il est d�fini en local donc il est supprim� apr�s le if. Faut plut�t d�finir un tableau dynamique avant, et d�finir sa taille dans le if avec un new)
            }
        }
