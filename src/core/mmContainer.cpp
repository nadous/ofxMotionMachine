...
bool TimedData::checkTimeVec(arma::vec pTime, unsigned int pLastId)
{
	...
	it2 = it1 + 1;
	while (it2 != pTime.end())
	{ //verify if pTIme is monotonic increasing vector
...
{
	if (!mTimed || !mIsRealTime)
		return false;
	if (pTime <= this->maxTime())
		return false;
	mLastId++;
...
bool TimedVec::push(const arma::vec &pData)
{
	...
	else if (mLastId * (mLastId != mBufferSize) + pData.n_cols >= mBufferSize)
...
bool TimedCube::checkInput(const arma::mat &pData)
{
	...
	if (mData.n_elem > 0 && (pData.n_rows != mData.n_rows || pData.n_cols != mData.n_cols))
		return (false);
	else
		return (true);
}
