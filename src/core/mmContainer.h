...
class TimedData
{
  ...
  protected:
    TimedData()
    {
        ...
        mFrameRate = -1.0f;
        mInitialTime = 0.f;
        mTimeVec = arma::zeros(0, 0);
        mInterpValidTime = 0.5;
        mBufferSize = 0;
        mLastId = 0;
        mIsRealTime = false;
        mBufferSize = 0;
  ...
  public:
    ...
    inline unsigned int setBufferSize(unsigned int pBufferSize)
    {
  ...
  protected:
    ...
    bool setValidIntervalTime(double time)
    {
        mInterpValidTime = time;
        return true;
    };
...
double TimedData::time(unsigned int index) const
{
    ...
    if (mTimed)
        return (mTimeVec[memIndex(index)]);
    else
        return (mInitialTime + ((double)index) / mFrameRate);
}
...
unsigned int TimedData::nearestIndex(double time) const
{
    if (!mTimed)
    {
        ...
        if (lIndex < 0)
            lIndex = 0;
        if (lIndex >= mBufferSize)
            lIndex = mBufferSize - 1;
        return (lIndex);
    }
    else
    {
...
double TimedData::maxTime(void) const
{
    ...
    if (mTimed)
        ...
        return (mIsFilled ? (mInitialTime + (double)(mBufferSize - 1) / mFrameRate) : (mInitialTime + (double)(mLastId % mBufferSize) / mFrameRate));
}
double TimedData::minTime(void) const
{
    // Return/compute time at the last index of time
    if (mTimed)
        return (mIsFilled ? mTimeVec.min() : mTimeVec.at(0));
    else
        return (mInitialTime);
}
...
arma::mat TimedCube::get(double pTime) const
{
    ...
    if (mTimed)
    {
        ...
        {
            if (validFlag)
                return ((weight1 * mData.slice(index1) + weight2 * mData.slice(index2)) / (weight1 + weight2));
            else
