...
Frame Track::frame(unsigned int index) const
{
    ...
    if (index < nOfFrames())
    {
        ...
        if (hasRotation)
        { // We do copy rotation information only if necessary
...
double Track::maxTime(void) const
{
    ...
    if (hasRotation)
        ...
        return (position.maxTime()); // Make a robust version of this return
}
double Track::minTime(void) const
{

    if (hasRotation)
        return (std::min(position.minTime(), rotation.minTime()));
    else
        return (position.minTime()); // Make a robust version of this return
}
...
unsigned int Track::nOfFrames(void) const
{
    ...
    if (hasRotation)
        ...
        return (position.nOfFrames()); // Make a robust version of this return
}
